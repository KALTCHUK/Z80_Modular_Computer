...
if ("-h" in opts) or (len(opts) == 0):
    print('Use: xmodem.py -p<port> [-d<drive>] -f<file>' + '\n\r')
...
print('Starting XMODEM.COM on CP/M.')
if noDrive == True:
    Z80_port.write(b'XMODEM ' + ufile.encode() + b'\r')
else:
    Z80_port.write(b'XMODEM ' + (drive + ':' + ufile).encode() + b'\r')

# Wait for <NAK>
print('Waiting for NAK... ', end='')
while True:
    rec_byte = Z80_port.read(1)
    if int.from_bytes(rec_byte, 'big') == NAK:
        print('Clear to go.')
        break
    if int.from_bytes(rec_byte, 'big') == CAN:
        print('Transmission canceled by receive')
        Z80_port.close()
        f.close()
        exit()
