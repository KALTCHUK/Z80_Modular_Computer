/*
 * preTTY.c originated from CStest.c
 *
 * Created: 29/08/2021
 * Author : kaltchuk
 */ 
...
{
	if (modus == asInput)	// Write zeroes to PORTs
	{
		PORTB &= ~0x07;
		PORTD &= ~0xf8;
	}
...
int main(void)
{
	...
	sei();
    while (1) 				// If we're not busy atending a service request
